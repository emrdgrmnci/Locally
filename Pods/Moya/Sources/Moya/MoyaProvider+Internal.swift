...
public extension MoyaProvider {
    ...
    func requestNormal(_ target: Target,
        ...
        let performNetworking = { (requestResult: Result<URLRequest, MoyaError>) in
            ...
            cancellableToken.innerCancellable = self.performRequest(
    ...
    final func createStubFunction(_ token: CancellableToken,
                                  ...
                                  endpoint: Endpoint,
                                  plugins: [PluginType],
                                  request: URLRequest) -> (() -> Void) { // swiftlint:disable:this function_parameter_count
        return {
...
private extension MoyaProvider {
    func sendUploadMultipart(_ target: Target,
                             ...
                             completion: @escaping Moya.Completion) -> CancellableWrapper {
        ...
        manager.upload(multipartFormData: multipartFormData, with: request) { result in
            ...
            case .success(let alamoRequest, _, _):
                ...
                let validatedRequest = validationCodes.isEmpty ? alamoRequest
                    : alamoRequest.validate(statusCode: validationCodes)
                cancellable.innerCancellable = self.sendAlamofireRequest(validatedRequest,
                                                                         target: target,
                                                                         callbackQueue: callbackQueue,
                                                                         progress: progress,
                                                                         completion: completion)
    ...
    func sendUploadFile(_ target: Target,
        ...
        return self.sendAlamofireRequest(alamoRequest,
                                         target: target,
                                         callbackQueue: callbackQueue,
                                         progress: progress,
                                         completion: completion)
